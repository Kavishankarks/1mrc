...
impl LoadTestClient {
    fn new(base_url: &str) -> Self {
        let client = reqwest::Client::builder()
            .pool_max_idle_per_host(20) // Conservative connection pool
    ...
    async fn run_load_test(&self, total_requests: usize, batch_size: usize, max_concurrent_batches: usize) {
        println!("🚀 Starting HTTP/2 load test");
        ...
        match self.get_stats().await {
            Ok(stats) => {
                println!("✅ Connected! Initial stats: {:?}", stats);
                println!("🌐 Using HTTP/2 with connection pooling");
            },
            Err(e) => {
    ...
    fn print_results(&self, duration: Duration, server_stats: &Stats) {
        ...
        println!("\n{}", "=".repeat(60));
        println!("🎯 HTTP/2 LOAD TEST RESULTS");
        println!("{}", "=".repeat(60));
...
async fn main() {
    ...
    println!("\n{}", "=".repeat(60));
    println!("🎯 MAIN CHALLENGE: 100,000 REQUESTS");
    println!("{}", "=".repeat(60));
    
    // Start with 100K to test, then scale up
    client.run_load_test(100_000, 200, 25).await;
}